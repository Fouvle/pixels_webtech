    ...
    <div class="grid md:grid-cols-4 gap-6">
      ...
      if ($result && $result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
          ...
          echo '<img src="../assets/' . htmlspecialchars($row['image_url'], ENT_QUOTES, 'UTF-8') . '" alt="' . htmlspecialchars($row['productName'], ENT_QUOTES, 'UTF-8') . '" class="w-full h-full object-cover rounded-lg" />';
          echo '</div>';
          echo '<p class="text-sm text-gray-600">' . htmlspecialchars($row['productName'], ENT_QUOTES, 'UTF-8') . '</p>';
          echo '<h3 class="font-medium">' . htmlspecialchars($row['productName'], ENT_QUOTES, 'UTF-8') . '</h3>';
          echo '<div class="flex items-center gap-1">';
          echo '<svg class="h-4 w-4 fill-yellow-400 text-yellow-400">...</svg>';
          echo '<span class="text-sm text-gray-600">' . htmlspecialchars($row['rating'], ENT_QUOTES, 'UTF-8') . '</span>';
          echo '</div>';
          echo '<p class="font-medium">$' . htmlspecialchars($row['price'], ENT_QUOTES, 'UTF-8') . '</p>';
          echo '</div>';
        }
      } else {
